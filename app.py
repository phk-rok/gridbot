...
# ---------- Config & Modes ----------
DEFAULT_TEST_MODE = os.getenv("TEST_MODE", "true").lower() == "true"   # initial default
...
PRICE_LOW = os.getenv("PRICE_LOW")    # None -> auto from price
...
# instantiate both feeds so we can toggle at runtime
try:
...
# ---------- Strategy Presets (up/middle/down) ----------
STRATEGY_PROFILES = {
    "up": {
        "name": "Up (상승장)",
        "up_pct": 0.025,    # +2.5%
                        ...
                        with state_lock:
                            ...
                            # --- 전략 프리셋 변경 ---
                                ...
                                if key and key in STRATEGY_PROFILES:
                                    ...
                                    tg_send(
                                        f"현재 전략: {prof['name']} ({key})\n"
                                        f"범위: {int(low):,} ~ {int(high):,}\n"
                                        f"N_GRIDS: {n} | PADDING: {pad} | INTERVAL: {itv}s"
                                    )
                                else:
